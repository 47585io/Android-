...
public class FileList
{
	...
	public FileList()
	{
		...
		sortFilelist=EmptyArray.emptyArray(File.class);
		refreshDate();
	}
	...
	public void addAfile(String name)
	{
		...
		try{
			...
			GrowingArrayUtils.insert(sortFilelist,sortFilelistSize++,index,f);
	...
	{
		sortFilelist[index].delete();
		GrowingArrayUtils.remove(sortFilelist,sortFilelistSize,index);
		sortFilelistSize--;
		if(mFileListener!=null){
	...
	{
		int i = 0;
		for(;i<sortFilelistSize;++i)
		{
			File f2 = sortFilelist[i];
	...
	public static interface FileChangeLisrener
	{
		...
		public void addAFile(File[] files,int index)

		public void delAFile(File[] files,int index)
