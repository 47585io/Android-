...
/*
  ...
  self表示编辑器本身，canvas和paint分别表示编辑器的画布和画笔，x,y是一个坐标，它可能是编辑器上次被触摸的坐标，也可能是光标坐标，这由具体的编辑器决定
...
public abstract class myEditCanvaserListener extends myEditListener implements EditCanvaserListener
{
	...
	abstract protected void beforeDraw(View self,Canvas canvas,Paint paint,float x,float y);
	
	abstract protected void afterDraw(View self,Canvas canvas,Paint paint,float x,float y)
	
	
	@Override
	public void onDraw(View self, Canvas canvas, Paint paint, float x, float y, int flag)
	{
		if(flag == BeforeDraw){
		    beforeDraw(self,canvas,paint,x,y);
		}
		else if(flag == AfterDraw){
			afterDraw(self,canvas,paint,x,y);
		}
	}
