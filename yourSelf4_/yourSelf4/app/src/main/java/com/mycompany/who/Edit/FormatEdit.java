...
public class FormatEdit extends DrawerHTML
{
	...
	protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter)
	{
		...
		if (lengthAfter != 0)
		{   
			if (lengthAfter < 5 && lengthBefore == 0)
			{
				IsModify2=true;
				//如果输入字符过多，不立即format
				if (Enabled_Format)
	...
	public void startFormat(int start, int end)
	{
		...
		for (EditListener total:getFormatorList())
		{
			...
			try
			{
				nowIndex =((EditFormatorListener) total).dothing_Start(getText(), nowIndex);
	            for (;nowIndex < end && nowIndex != -1;)
				{
					beforeIndex = nowIndex;
			        nowIndex = ((EditFormatorListener) total).dothing_Run(getText(), nowIndex);
		        }
				nowIndex = ((EditFormatorListener) total).dothing_End(getText(), beforeIndex);
		  	}
			catch (Exception e)
			{}
		}
